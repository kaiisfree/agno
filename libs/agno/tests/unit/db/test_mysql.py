from unittest.mock import Mock, patch

import pytest
from sqlalchemy.engine import Engine
from sqlalchemy.orm import Session
from sqlalchemy.schema import Index, Table, UniqueConstraint

from agno.db.mysql.mysql import MySQLDb
from agno.db.mysql.schemas import (
    CULTURAL_KNOWLEDGE_TABLE_SCHEMA,
    EVAL_TABLE_SCHEMA,
    KNOWLEDGE_TABLE_SCHEMA,
    METRICS_TABLE_SCHEMA,
    SESSION_TABLE_SCHEMA,
    SPAN_TABLE_SCHEMA,
    TRACE_TABLE_SCHEMA,
    USER_MEMORY_TABLE_SCHEMA,
    VERSIONS_TABLE_SCHEMA,
    get_table_schema_definition,
)


@pytest.fixture
def mock_engine():
    """Create a mock SQLAlchemy engine"""
    engine = Mock(spec=Engine)
    engine.url = "fake:///url"
    return engine


@pytest.fixture
def mock_session():
    """Create a mock session"""
    session = Mock(spec=Session)
    session.__enter__ = Mock(return_value=session)
    session.__exit__ = Mock(return_value=None)
    session.begin = Mock()
    session.begin().__enter__ = Mock(return_value=session)
    session.begin().__exit__ = Mock(return_value=None)
    return session


@pytest.fixture
def mysql_db(mock_engine):
    """Create a MySQLDb instance with mock engine"""
    return MySQLDb(
        db_engine=mock_engine,
        db_schema="test_schema",
        session_table="test_sessions",
        memory_table="test_memories",
        metrics_table="test_metrics",
        eval_table="test_evals",
        knowledge_table="test_knowledge",
    )


# -- Initialization tests --


def test_id_is_deterministic(mock_engine):
    """Test that database IDs are deterministic for the same engine"""
    first_db = MySQLDb(db_engine=mock_engine)
    second_db = MySQLDb(db_engine=mock_engine)
    assert first_db.id == second_db.id


def test_init_with_engine(mock_engine):
    """Test initialization with engine"""
    db = MySQLDb(db_engine=mock_engine, session_table="sessions")
    assert db.db_engine == mock_engine
    assert db.db_schema == "ai"
    assert db.session_table_name == "sessions"


@patch("agno.db.mysql.mysql.create_engine")
def test_init_with_url(mock_create_engine):
    """Test initialization with database URL"""
    mock_engine = Mock(spec=Engine)
    mock_create_engine.return_value = mock_engine

    db = MySQLDb(db_url="mysql://user:pass@localhost/db", session_table="sessions")

    mock_create_engine.assert_called_once_with("mysql://user:pass@localhost/db")
    assert db.db_engine == mock_engine


def test_init_no_engine_or_url():
    """Test initialization fails without engine or URL"""
    with pytest.raises((ValueError, AttributeError)):
        MySQLDb(session_table="sessions")


def test_init_no_tables(mock_engine):
    """Test initialization works when not specifying any tables"""
    MySQLDb(db_engine=mock_engine)


# -- Schema definition tests --


def test_get_table_schema_definition_sessions():
    """Test getting session table schema"""
    schema = get_table_schema_definition("sessions")
    assert schema == SESSION_TABLE_SCHEMA
    assert "session_id" in schema
    assert schema["session_id"]["nullable"] is False
    assert "_unique_constraints" in schema


def test_get_table_schema_definition_memories():
    """Test getting memory table schema"""
    schema = get_table_schema_definition("memories")
    assert schema == USER_MEMORY_TABLE_SCHEMA
    assert "memory_id" in schema
    assert schema["memory_id"]["primary_key"] is True


def test_get_table_schema_definition_evals():
    """Test getting eval table schema"""
    schema = get_table_schema_definition("evals")
    assert schema == EVAL_TABLE_SCHEMA
    assert "run_id" in schema
    assert schema["eval_type"]["nullable"] is False


def test_get_table_schema_definition_knowledge():
    """Test getting knowledge table schema"""
    schema = get_table_schema_definition("knowledge")
    assert schema == KNOWLEDGE_TABLE_SCHEMA
    assert "id" in schema
    assert schema["name"]["nullable"] is False


def test_get_table_schema_definition_metrics():
    """Test getting metrics table schema"""
    schema = get_table_schema_definition("metrics")
    assert schema == METRICS_TABLE_SCHEMA
    assert "date" in schema
    assert schema["date"]["index"] is True
    assert "_unique_constraints" in schema


def test_get_table_schema_definition_culture():
    """Test getting culture table schema"""
    schema = get_table_schema_definition("culture")
    assert schema == CULTURAL_KNOWLEDGE_TABLE_SCHEMA
    assert "id" in schema
    assert schema["id"]["primary_key"] is True


def test_get_table_schema_definition_versions():
    """Test getting versions table schema"""
    schema = get_table_schema_definition("versions")
    assert schema == VERSIONS_TABLE_SCHEMA
    assert "table_name" in schema
    assert schema["table_name"]["primary_key"] is True


def test_get_table_schema_definition_traces():
    """Test getting traces table schema"""
    schema = get_table_schema_definition("traces")
    assert schema == TRACE_TABLE_SCHEMA
    assert "trace_id" in schema
    assert schema["trace_id"]["primary_key"] is True


def test_get_table_schema_definition_spans():
    """Test getting spans table schema"""
    schema = get_table_schema_definition("spans")
    assert schema == SPAN_TABLE_SCHEMA
    assert "span_id" in schema
    assert "trace_id" in schema
    assert "foreign_key" in schema["trace_id"]


def test_get_table_schema_definition_invalid():
    """Test getting schema for invalid table type"""
    with pytest.raises(ValueError, match="Unknown table type"):
        get_table_schema_definition("invalid_table")


# -- Table creation tests --


def test_create_table(mysql_db, mock_session):
    """Test table creation"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "create") as mock_table_create:
        with patch("agno.db.mysql.mysql.create_schema") as mock_create_schema:
            with patch("agno.db.mysql.mysql.is_table_available", return_value=False):
                table = mysql_db._create_table("test_sessions", "sessions")

    mock_create_schema.assert_called()
    mock_table_create.assert_called_with(mysql_db.db_engine, checkfirst=True)

    assert table.name == "test_sessions"
    assert table.schema == "test_schema"

    column_names = [col.name for col in table.columns]
    expected_columns = [
        "session_id",
        "session_type",
        "agent_id",
        "team_id",
        "workflow_id",
        "user_id",
        "session_data",
        "agent_data",
        "team_data",
        "workflow_data",
        "metadata",
        "runs",
        "summary",
        "created_at",
        "updated_at",
    ]
    for col in expected_columns:
        assert col in column_names


def test_create_table_with_indexes(mysql_db, mock_session):
    """Test table creation with indexes"""
    mysql_db.Session = Mock(return_value=mock_session)
    mock_session.execute.return_value.scalar.return_value = None

    with patch.object(Table, "create"):
        with patch.object(Index, "create"):
            with patch("agno.db.mysql.mysql.create_schema"):
                table = mysql_db._create_table("test_metrics", "metrics")

    for index in table.indexes:
        column = index.columns[0]
        assert column.name == "date"


def test_create_table_with_unique_constraints(mysql_db, mock_session):
    """Test table creation with unique constraints"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "create"):
        with patch("agno.db.mysql.mysql.create_schema"):
            table = mysql_db._create_table("test_metrics", "metrics")

    constraint_names = [c.name for c in table.constraints if isinstance(c, UniqueConstraint)]
    assert "test_metrics_uq_metrics_date_period" in constraint_names

    for constraint in table.constraints:
        if isinstance(constraint, UniqueConstraint) and constraint.name == "test_metrics_uq_metrics_date_period":
            col_names = [col.name for col in constraint.columns]
            assert "date" in col_names
            assert "aggregation_period" in col_names


def test_create_memory_table(mysql_db, mock_session):
    """Test creation of memory table with correct schema"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "create"):
        with patch("agno.db.mysql.mysql.create_schema"):
            table = mysql_db._create_table("test_memories", "memories")

    pk_columns = [col.name for col in table.columns if col.primary_key]
    assert "memory_id" in pk_columns

    indexed_columns = []
    for index in table.indexes:
        for column in index.columns:
            indexed_columns.append(column.name)
    assert set(indexed_columns) == {"user_id", "created_at", "updated_at"}


def test_create_eval_table(mysql_db, mock_session):
    """Test creation of eval table with correct schema"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "create"):
        with patch("agno.db.mysql.mysql.create_schema"):
            table = mysql_db._create_table("test_evals", "evals")

    column_names = [col.name for col in table.columns]
    assert "run_id" in column_names
    assert "eval_type" in column_names
    assert "eval_data" in column_names

    pk_columns = [col.name for col in table.columns if col.primary_key]
    assert "run_id" in pk_columns


def test_create_knowledge_table(mysql_db, mock_session):
    """Test creation of knowledge table with correct schema"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "create"):
        with patch("agno.db.mysql.mysql.create_schema"):
            table = mysql_db._create_table("test_knowledge", "knowledge")

    column_names = [col.name for col in table.columns]
    expected_columns = [
        "id",
        "name",
        "description",
        "metadata",
        "type",
        "size",
        "linked_to",
        "access_count",
        "created_at",
        "updated_at",
        "status",
        "status_message",
    ]
    for col in expected_columns:
        assert col in column_names


# -- _get_table tests --


def test_get_table_sessions(mysql_db):
    """Test getting sessions table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("sessions")
    assert table == mock_table
    assert hasattr(mysql_db, "session_table")


def test_get_table_memories(mysql_db):
    """Test getting memories table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("memories")
    assert table == mock_table
    assert hasattr(mysql_db, "memory_table")


def test_get_table_metrics(mysql_db):
    """Test getting metrics table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("metrics")
    assert table == mock_table
    assert hasattr(mysql_db, "metrics_table")


def test_get_table_evals(mysql_db):
    """Test getting evals table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("evals")
    assert table == mock_table
    assert hasattr(mysql_db, "eval_table")


def test_get_table_knowledge(mysql_db):
    """Test getting knowledge table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("knowledge")
    assert table == mock_table
    assert hasattr(mysql_db, "knowledge_table")


def test_get_table_culture(mysql_db):
    """Test getting culture table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("culture")
    assert table == mock_table
    assert hasattr(mysql_db, "culture_table")


def test_get_table_versions(mysql_db):
    """Test getting versions table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("versions")
    assert table == mock_table
    assert hasattr(mysql_db, "versions_table")


def test_get_table_traces(mysql_db):
    """Test getting traces table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("traces")
    assert table == mock_table
    assert hasattr(mysql_db, "traces_table")


def test_get_table_spans(mysql_db):
    """Test getting spans table"""
    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_get_or_create_table", return_value=mock_table):
        table = mysql_db._get_table("spans")
    assert table == mock_table
    assert hasattr(mysql_db, "spans_table")


def test_get_table_invalid_type(mysql_db):
    """Test getting table with invalid type"""
    with pytest.raises(ValueError, match="Unknown table type"):
        mysql_db._get_table("invalid_type")


# -- _get_or_create_table tests --


@patch("agno.db.mysql.mysql.is_table_available")
@patch("agno.db.mysql.mysql.is_valid_table")
def test_get_or_create_table_existing_valid(mock_is_valid, mock_is_available, mysql_db, mock_session):
    """Test getting existing valid table"""
    mock_is_available.return_value = True
    mock_is_valid.return_value = True
    mysql_db.Session = Mock(return_value=mock_session)

    mock_table = Mock(spec=Table)
    with patch.object(Table, "__new__", return_value=mock_table):
        table = mysql_db._get_or_create_table("test_table", "sessions")

    assert table == mock_table
    mock_is_available.assert_called_once()
    mock_is_valid.assert_called_once()


@patch("agno.db.mysql.mysql.is_table_available")
def test_get_or_create_table_not_available(mock_is_available, mysql_db, mock_session):
    """Test creating table when not available"""
    mock_is_available.return_value = False
    mysql_db.Session = Mock(return_value=mock_session)
    mysql_db.upsert_schema_version = Mock(return_value=None)

    mock_table = Mock(spec=Table)
    with patch.object(mysql_db, "_create_table", return_value=mock_table):
        table = mysql_db._get_or_create_table(
            table_name="test_table", table_type="sessions", create_table_if_not_found=True
        )
        assert table == mock_table
        mysql_db._create_table.assert_called_once_with(table_name="test_table", table_type="sessions")


@patch("agno.db.mysql.mysql.is_table_available")
def test_get_or_create_table_not_available_no_create(mock_is_available, mysql_db, mock_session):
    """Test returning None when table not available and create_table_if_not_found is False"""
    mock_is_available.return_value = False
    mysql_db.Session = Mock(return_value=mock_session)

    table = mysql_db._get_or_create_table(
        table_name="test_table", table_type="sessions", create_table_if_not_found=False
    )
    assert table is None


@patch("agno.db.mysql.mysql.is_table_available")
@patch("agno.db.mysql.mysql.is_valid_table")
def test_get_or_create_table_invalid_schema(mock_is_valid, mock_is_available, mysql_db, mock_session):
    """Test error when table exists but has invalid schema"""
    mock_is_available.return_value = True
    mock_is_valid.return_value = False
    mysql_db.Session = Mock(return_value=mock_session)

    with pytest.raises(ValueError, match="has an invalid schema"):
        mysql_db._get_or_create_table("test_table", "sessions")


@patch("agno.db.mysql.mysql.is_table_available")
@patch("agno.db.mysql.mysql.is_valid_table")
def test_get_or_create_table_load_error(mock_is_valid, mock_is_available, mysql_db, mock_session):
    """Test error when loading existing table fails"""
    mock_is_available.return_value = True
    mock_is_valid.return_value = True
    mysql_db.Session = Mock(return_value=mock_session)

    with patch.object(Table, "__new__", side_effect=Exception("Load error")):
        with pytest.raises(Exception):
            mysql_db._get_or_create_table("test_table", "sessions")


# -- table_exists tests --


def test_table_exists(mysql_db, mock_session):
    """Test table_exists delegates to is_table_available"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch("agno.db.mysql.mysql.is_table_available", return_value=True) as mock_check:
        result = mysql_db.table_exists("my_table")

    assert result is True
    mock_check.assert_called_once_with(session=mock_session, table_name="my_table", db_schema="test_schema")


def test_table_exists_false(mysql_db, mock_session):
    """Test table_exists returns False when table does not exist"""
    mysql_db.Session = Mock(return_value=mock_session)

    with patch("agno.db.mysql.mysql.is_table_available", return_value=False):
        result = mysql_db.table_exists("nonexistent_table")

    assert result is False
